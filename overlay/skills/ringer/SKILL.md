...
description: >-
  ...
  reviewing a manifest, choosing a swarm pattern, picking a worker engine, or
...
2. **A single task is a one-task manifest.** Same verification, zero
   ...
   the probe script, the three-edit fix are all one-task manifests. (The
   ONLY exception: the user's routing words — "workflow"/"native swarm"/
   "inline" — re-route that job; honor them absolutely, no re-offering.)
...
./ringer.py run manifest.json --baseline  # execute checks, spawn no workers
...
`engine`/`model`/`engine_args` fields; don't re-ask every round of the same job unless
