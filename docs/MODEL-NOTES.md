...
- 2026-08-02 (bids alex-plain-language round, filed here for cross-reference; model was SOL not kimi) — codex/gpt-5.6-sol went 4/4 first-try on plain-language rewrite tasks (voice bans + structure/fact-freeze checks): answers 167k/15m, overview 142k/14m, manual 52k/4m, reference YAMLs 116k/13m. Well above its 59% research baseline; heavily-constrained rewrite work suits it. Residual tells the checks missed and the orchestrator fixed by hand: bracketed section prefixes on every heading, one entity-type slip (basis values described as columns), one direct-address "Alex," insertion. Add "no bracketed heading prefixes" to future voice checks.
- 2026-08-02 (bids alex-gaps deliverables round) — first post-retirement production run: opencode `kimi-for-coding/k3`, 2/2 first-try. answers-docx re-issue (research, 26-question doc with mechanically-checked judgment-call block): 84k tokens, 24m. pipeline_overview full prose rewrite (docs, placeholder/anchor-preserving, stale-fact bans): 56k tokens, 10m. Both self-verified against executed checks before finishing. One content defect the check couldn't see: it truthfully reported its sandbox's 42 PermissionError test failures as the project's suite status — orchestrator caught it on spot-check. Lesson: specs for read-only workers should say "report the suite count from the repo's own records, do not run the suite in your sandbox".

## Check-design lesson (2026-08-02, opencode-model-audition)

- doc_check.py's symbol validation demands byte-literal presence in source; combined with a spec saying "wrap every function you name in backticks", natural doc phrasings (`main(argv)`, `MOCK_FILE: <path>`) fail. Three of four docs lanes burned a retry on this exact trap. Next docs manifest: either seed the spec with the literal-match rule AND an example, or pass a --symbol-allowlist for signature-style phrasings. Retry-with-injected-failure absorbed it every time, so pass_rate survived — but first-try stats for docs undercount the models. FIXED 2026-08-02: doc_check now has --symbol-mode relaxed (implemented by BOTH round-2 heavyweights against a hash-guarded contract; opus's patch landed).

## Round 2 — Sonnet succession + Opus/Sol (2026-08-02, run round2-heavyweight-succession)

- **Check-craft first (two more lessons, both cost cells):** (1) quote-grounding checks must strip backslash-escapes and use a tempered quote regex — JSON-heavy sources (OpenAI/models.dev pages) made every competitor quote escaped JSON, and all 3 research cells failed on MY regex, not on substance (sonnet's artifact passed 30+30 clean once the check was fixed); (2) review checks must not fail out-of-diff citations — opus cited config.sample.toml as legitimate context and was wrongly failed twice. Fixed both, validated against the failed artifacts, re-ran 4 cells for clean rows.
- **sonnet** — research: first-try clean under the fair check (132s) on top of a proven 70%/20-task history; research is the ONE lane Sonnet keeps. code-fix and docs: first-try, but matched by cheaper lanes (see below) → per Austin's minimize-Sonnet directive, Sonnet routes ONLY research (and break-glass) from here; unplug decision after a round-3 confirm.
- **gpt-5.6-terra** — underwhelming for the mid-tier price point: code-fix first-try, but docs needed a retry, and research failed a citation-bookkeeping contract twice before passing on the rerun's attempt 2 at 263k tokens. No lane where it beats both Sonnet and the cheap lanes. Keep on the bench; re-audition when something changes.
